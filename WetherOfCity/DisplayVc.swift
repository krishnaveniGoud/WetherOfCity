...
class DisplayVc: UIViewController,CLLocationManagerDelegate,UITextFieldDelegate {
    ...
    func getReport(){
        ...
        geocoder.geocodeAddressString(app.cityName as String, completionHandler: {(placemarks, error) -> Void in
            if((error) != nil){
                print("Error", error)
                var alert = UIAlertController(title: "Error", message: "PleaceCkeckYourNetWork", preferredStyle: UIAlertControllerStyle.Alert)
            ...
            if let placemark: AnyObject = placemarks?.first {
                ...
                var  baseURL = NSURL(string: "https://api.forecast.io/forecast/\(apiKey)/\(coordinates.latitude),\(coordinates.longitude)")
                
                var dataObject = NSData(contentsOfURL: baseURL!)
                let weatherDictionary : NSDictionary = NSJSONSerialization.JSONObjectWithData( dataObject!, options: nil, error: nil) as! NSDictionary
                var dir: AnyObject? = weatherDictionary.objectForKey("currently")
                print("dir \(dir)")
                var dig = dir!.valueForKey("temperature") as! Int
                print("dig \(dig)")
                var  temperature = self.convertToCelsius(dig)
